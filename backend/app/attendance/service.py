...
def get_daily(db: Session, target_date: date) -> AttendanceDailyResponse:
    ...
    for att, name, emp_code, required_hours in rows:
        items.append(AttendanceDailyItem(
            id=att.id,
    ...
    return AttendanceDailyResponse(date=target_date, total=len(items), items=items)
...
def get_monthly(db: Session, year: int, month: int) -> AttendanceMonthResponse:
    ...
    for row in rows:
        ...
        emp_map[key].days[day_num] = row.status
        if row.status in PAID_STATUSES:
